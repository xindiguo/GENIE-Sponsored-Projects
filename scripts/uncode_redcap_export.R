# Description: Uncode REDCap export with the option of merge
#   all exported files from all sites
...
choices_project <- sort(names(config$upload))
choices_site <- sort(unique(unlist(lapply(config$upload, names))))
choice_all = "all"
...
parser$add_argument("-p", dest = "project", type = "character", choices = choices_project, 
                    help = "Project on which to perform checks", required = T)
...
parser$add_argument("-m", "--merge", action="store_true", help="Merge the files after uncoding")       
parser$add_argument("-u", "--save_synapse", action="store_true", help="Save output to Synapse")
parser$add_argument("-a", "--synapse_auth", type = "character", default = "~/.synapseConfig",
...
site <- args$site
merge <- args$merge
save_on_synapse <- args$save_synapse
auth <- args$synapse_auth
debug <- args$verbose
waitifnot(!is.null(args$project),
...
waitifnot(is.element(project, choices_project), 
            msg = glue("Error: project '{project}' invalid.  Valid values: {choices_project}."))
...
#' Return a file upload data corresponding to a project-site pair as an R object.
#'
...
#' @example
#' get_data_upload(project, site)
get_data_upload <- function(project, site) {
  ent <- synGet(config$upload[[project]][[site]])
  data <- read.csv(ent$path, check.names = F,
                   na.strings = c(""), 
                   stringsAsFactors = F,
...
#' Get Synapse IDs of the most recent non-PHI data dictionary or import
#' template files for a project.
#'
#' @param project project of interest
...
#' to the project and type of document.
#' @example
#' get_dd_synid("my_project", "Import Template")
get_dd_synid <- function(project,
        file_name = c("Import Template", "Data Dictionary non-PHI")) {

  synid_folder_id <- config$synapse$data_dict_folder[[project]]
  
  synid_folder_children <- as.list(synGetChildren(synid_folder_id))
  synid_folder_children_ids <- setNames(unlist(lapply(synid_folder_children,
                                          function(x) {return(x$id)})),
                                        unlist(lapply(synid_folder_children,
                                          function(x) {return(x$name)})))
  
  return(as.character(synid_folder_children_ids[file_name]))
}
...
merge_datasets <- function(datasets, project, site) {
  
  mod <- list()
  config_project <- config$upload[[project]]
  
  for (i in 1:length(datasets)) {
    mod[[i]] <- data.frame(lapply(get_data_upload(project = project, 
                                          site = site, 
                                          obj = config_project[[i]]), as.character), 
                           stringsAsFactors = F)
  }
...
remove_irr <- function(data) {
  ...
  irr_cases <- grep('-2$|_2$|DA$', unique(data$record_id), value = T)
...
get_irr <- function(data) {
  ...
  if (length(irr_cases)) {
    ...
    irr_plus_cases <- c(irr_cases, gsub(x = irr_cases, pattern = '-2$|_2$|DA$', replacement = ''))
...
get_data_dictionary <- function(project) {
  synid_dd <- get_dd_synid(project = project,
...
get_data_uploads <- function(project) {
  data_upload <- list()
  for (i in 1:length(config$upload[[project]])) {
    
    config_project <- config$upload[[project]]
    site <- names(config_project)[i]
    
    data_upload[[site]] <- get_data_upload(project = project, 
                                               site = site, 
                                               obj = config_project[[site]])
  }
  
  return(data_upload)
}

get_pri_file_name <- function(project, site) {
  if(site=='all'){
    return(glue("{project}_labelled_data.csv"))
  }else{
    return(glue("{site}_{project}_labelled_data.csv"))
  }
}

get_irr_file_name <- function(project, site) {
  if(site=='all'){
    return(glue("{project}_labelled_irr.csv"))
  }else{
    return(glue("{site}_{project}_labelled_irr.csv"))
  }
}

#' Write to file(s) locally
write_output_locally <- function(project, site, data_pri, data_irr) {
  
  files_output <- c()
  
  file_output_pri <- get_pri_file_name(project, site)
  files_output <- append(files_output, file_output_pri)
  write.csv(x = data_pri, file = file_output_pri, row.names = F, na = "")
  
  if(!is.null(data_irr)) {
    file_output_irr <- get_irr_file_name(project, site)
...
save_output_synapse <- function(project, site, merge) {
  
  parent_id <- config$label_folder[[project]]
  file_output_pri <- get_pri_file_name(project,site)
  file_output_irr <- get_irr_file_name(project,site)
  synid_dd <- get_dd_synid(project = project, file_name = "Data Dictionary non-PHI")
  if(merge){
    prov_upload_used <- as.character(unlist(config$upload[[project]]))
  }else{
    prov_upload_used <- config$upload[[project]][[site]]
  }
  
  save_to_synapse(path = file_output_pri,
                  file_name = gsub(pattern = ".csv|.tsv", replacement = "", x = file_output_pri),
                  parent_id = parent_id,
                  prov_name = "non-IRR upload data",
                  prov_desc = "Uncoded upload data from sites academic REDCap instances with IRR cases removed",
                  prov_used = prov_upload_used,
  ...
  if (file.exists(file_output_irr)) {
    save_to_synapse(path = file_output_irr,
                    ...
                    prov_desc = "Uncoded upload IRR case data from sites academic REDCap instances",
                    prov_used = prov_upload_used,
                    prov_exec = "https://github.com/Sage-Bionetworks/GENIE-Sponsored-Projects/blob/develop/scripts/uncode_redcap_export.R")
  }
...
# for each user-specified project
if (debug) {
    print(glue("{now(timeOnly = T)}: Uncoding data for project {project} -------------------"))
    print(glue("{now(timeOnly = T)}: Reading data dictionary..."))
}
  
dd <- get_data_dictionary(project)
...
if (merge) {  
    data_upload <- get_data_uploads(project)
    if (debug) {
      print(glue("{now(timeOnly = T)}: Merging data uploads..."))
    }
    coded <- merge_datasets(data_upload, project)
}else{
    coded <- get_data_upload(project, site)
}
...
# uncode
uncoded <- uncode_data(df_coded = coded, dd = dd, grs = grs)
...
write_output_locally(project, site, data_pri, data_irr)
...
if (save_on_synapse) {
  ...
  save_output_synapse(project, site, merge)
}
